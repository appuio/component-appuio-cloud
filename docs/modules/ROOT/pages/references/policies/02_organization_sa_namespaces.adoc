...
[abstract]
--
Ensure that all namespaces created by organization serviceaccounts have a label `appuio.io/organization` which is not empty.
--
...
metadata:
  annotations:
    ...
    policies.kyverno.io/description: 'This policy will:
      ...
      permissions has a label appuio.io/organization which is not empty.
    ...
    policies.kyverno.io/title: Ensure that all namespaces created by organization
      serviceaccounts have a label `appuio.io/organization` which is not empty.
  labels:
